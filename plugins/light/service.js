    ...
    function LightService($http, $translate) {
        ...
        service.performUpdate = function(spokenWords){
            ...
            for(var i = 0; i < spokenWords.length; i++){
                ...
                if(spokenWords[i] == 'umbreon' || $translate.instant('lights.action.off') == spokenWords[i]){
                    SaidParameter['on'] = false;
                }

                // Choose Color
                if(spokenWords[i] == 'flareon' || $translate.instant('lights.colors.red') == spokenWords[i]){
                    SaidParameter['colorRGB'] = [255, 0, 0];
                    SaidParameter['colorHSV'] = [0, 255, 254];
                } else if(spokenWords[i] == 'leafeon' || $translate.instant('lights.colors.green') == spokenWords[i]){
                    SaidParameter['colorRGB'] = [0, 255, 0];
                    SaidParameter['colorHSV'] = [25500, 255, 254];
                } else if(spokenWords[i] == 'vaporeon' || $translate.instant('lights.colors.blue') == spokenWords[i]){
                    SaidParameter['colorRGB'] = [0, 0, 255];
                    SaidParameter['colorHSV'] = [46920, 255, 254];
                } else if(spokenWords[i] == 'jolteon' || $translate.instant('lights.colors.yellow') == spokenWords[i]){
                ...
                } else if(spokenWords[i] == 'sylveon' || $translate.instant('lights.colors.pink') == spokenWords[i]){
                    SaidParameter['colorRGB'] = [255, 0, 255];
                    SaidParameter['colorHSV'] = [54610, 255, 254];
                } else if(spokenWords[i] == 'espeon' || $translate.instant('lights.colors.purple') == spokenWords[i]){
                    SaidParameter['colorRGB'] = [127, 0, 127];
                    SaidParameter['colorHSV'] = [54610, 255, 127];
                } else if(spokenWords[i] == 'eevee' || $translate.instant('lights.colors.white') == spokenWords[i]){
